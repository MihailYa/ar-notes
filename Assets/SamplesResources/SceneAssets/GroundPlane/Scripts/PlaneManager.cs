...
public class PlaneManager : MonoBehaviour
{
    ...
    public void PlaceObjectInMidAir(Transform midAirTransform)
    {
        if (CurrentPlaneMode == PlaneMode.MIDAIR)
        {
            ...
            {
                GameObject newNoteObject = Instantiate(midAirAugmentation.transform.parent.gameObject);
                
                AnchorBehaviour newNoteObjectAnchorBehaviour = newNoteObject.GetComponent<AnchorBehaviour>();
                this.contentPositioningBehaviour.AnchorStage = newNoteObjectAnchorBehaviour;
                this.contentPositioningBehaviour.PositionContentAtMidAirAnchor(midAirTransform);
                GameObject visibleGameObject = newNoteObject.transform.GetChild(0).gameObject;
                UtilityHelper.EnableRendererColliderCanvas(visibleGameObject, true);

                visibleGameObject.transform.localPosition = Vector3.zero;
                RotatePlaneTowardCamera(visibleGameObject);
            }
        }
    }
    
    public static void RotatePlaneTowardCamera(GameObject augmentation)
    {
        if (Vuforia.VuforiaManager.Instance.ARCameraTransform != null)
        {
            ...
            augmentation.transform.Rotate(Vector3.right, 90);
        }
    }
    ...
    void SetMode(PlaneMode mode)
    {
        ...
        /*this.touchHandler.enableRotation = 
        (mode == PlaneMode.PLACEMENT) &&
    ...
    void SetNoteGameObject(GameObject noteGameObject)
    {
        midAirAugmentation = noteGameObject;
    }
}
